...
| **Optimistic concurrency (`If-Match`)** | The first report of a silently lost edit, or the first screen where two people routinely edit one record. Shape: a `lock_version` column on the contended tables, bumped on write, plus `If-Match` on their update routes and a 412 when it does not match. Everything is last-write-wins today; the row locks keep the database consistent but cannot tell a writer that what they read has since changed. Noted because `01-architecture.md` asserted this existed before it was ever built — the assertion was removed in M10c, not the need. |
