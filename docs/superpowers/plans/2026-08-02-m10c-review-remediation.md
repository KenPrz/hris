...
2. **The meal break comes out of the overage, not out of the threshold.** *(Added during Task 1, approved by the repo owner.)* Moving the threshold from the scheduled day to the statutory 300 was not enough on its own: the assumed policy is a step function, so the cliff simply relocated to 5h01, where `MealBreakPolicyTest.php:13` pinned it as `netWorked(301) === 241`. An employee on the clock for one minute past five hours cannot have taken a sixty-minute lunch inside that minute. `MealBreakPolicy::netWorked` now floors at the threshold — `max(threshold, gross - break)` — so the deduction ramps in over the first `breakMinutes` of overage and is whole thereafter. Monotonic everywhere, and identical to the old behaviour for every span at or above `threshold + breakMinutes`, which is every full working day. Two pinned assertions in `MealBreakPolicyTest` changed with it.

3. **Approved leave pays exactly what it debits.** Today the ledger debit reads `offices.minutes_per_leave_day` and the pay line reads the employee's resolved `scheduledMinutes`; a 4×10 employee debits 480 and is paid 600. Task 5 snapshots one number onto `leave_details` at submit time and has both sides read it. The alternative — pay the scheduled length and debit the scheduled length — was rejected because it makes a leave balance mean different things for different employees.
