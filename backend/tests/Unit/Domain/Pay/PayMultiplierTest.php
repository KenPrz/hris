...
|
| PayMultiplier no longer hardcodes the matrix — it reads a PayRates argument. Every
| call below passes PayRatesFactory::statutory(), so the expected values are UNCHANGED
| from before the refactor: that equality is the proof the refactor is behavior-
| preserving. This file needs config('hris.pay_floors') (via PayRatesFactory), so it
| opts back into the booted TestCase the same way StatutoryFloorTest does — tests/Unit
| is deliberately unbooted otherwise (see tests/Pest.php).
*/
...
it('pays the statutory base rate for time worked', function (DayType $type, bool $restDay, int $expected): void {
    expect(PayMultiplier::forWorkedTime($type, $restDay, false, false, false, PayRatesFactory::statutory())->value)->toBe($expected);
...
it('does not derive the special non-working rest-day rate by formula', function (): void {
    ...
    expect(PayMultiplier::forWorkedTime(DayType::SpecialNonWorking, true, false, false, false, PayRatesFactory::statutory())->value)
        ->toBe(15000)
...
it('pays overtime at +25% on an ordinary day and +30% everywhere else', function (DayType $type, bool $restDay, int $expected): void {
    expect(PayMultiplier::forWorkedTime($type, $restDay, true, false, false, PayRatesFactory::statutory())->value)->toBe($expected);
})->with('overtime rates');
...
it('compounds night differential on the already-premium rate, not on base pay', function (): void {
    ...
    expect(PayMultiplier::forWorkedTime(DayType::RegularHoliday, false, false, true, false, PayRatesFactory::statutory())->value)
        ->toBe(22000);

    // Holiday overtime at 2am: 200% x 130% x 110%.
    expect(PayMultiplier::forWorkedTime(DayType::RegularHoliday, false, true, true, false, PayRatesFactory::statutory())->value)
        ->toBe(28600);

    // An ordinary night shift hour.
    expect(PayMultiplier::forWorkedTime(DayType::Ordinary, false, false, true, false, PayRatesFactory::statutory())->value)
        ->toBe(11000);
});
...
it('reaches 371.8% for the worst-case hour in the matrix', function (): void {
    ...
    expect(PayMultiplier::forWorkedTime(DayType::RegularHoliday, true, true, true, false, PayRatesFactory::statutory())->value)
        ->toBe(37180);
});
        ...
        foreach ([true, false] as $restDay) {
            foreach ([true, false] as $overtime) {
                foreach ([true, false] as $night) {
                    expect(PayMultiplier::forWorkedTime($type, $restDay, $overtime, $night, true, PayRatesFactory::statutory())->value)
                        ->toBe(10000);
                }
            }
        }
...
it('pays an Art. 82-exempt employee nothing extra for an unworked holiday', function (): void {
    expect(PayMultiplier::forUnworkedDay(DayType::RegularHoliday, true, PayRatesFactory::statutory())->value)->toBe(0)
        ->and(PayMultiplier::forUnworkedDay(DayType::DoubleRegularHoliday, true, PayRatesFactory::statutory())->value)->toBe(0);
});
...
it('pays an unworked regular holiday and nothing for an unworked special day', function (DayType $type, int $expected): void {
    ...
    expect(PayMultiplier::forUnworkedDay($type, false, PayRatesFactory::statutory())->value)->toBe($expected);
...
it('covers every day type, so a new one cannot be added without a rate', function (): void {
    foreach (DayType::cases() as $type) {
        expect(PayMultiplier::forWorkedTime($type, false, false, false, false, PayRatesFactory::statutory())->value)
            ->toBeGreaterThanOrEqual(10000)
            ->and(PayMultiplier::forUnworkedDay($type, false, PayRatesFactory::statutory())->value)->toBeGreaterThanOrEqual(0);
    }
});
